...
async def get_sensor_urls(labeled_device: LabeledDevice) -> tuple[str, str, str]:
	...
	# Prevent indefinite hang on status
	status = await asyncio.wait_for(labeled_device.device.get_status(), timeout=5.0)
	...
	if not sensor_gaze.connected:
		raise RuntimeError(
			f"Could not connect to direct gaze sensor for device {labeled_device.emission_id} ({labeled_device.label})"
		)
	...
	if not sensor_video.connected:
		raise RuntimeError(
			f"Could not connect to direct world sensor (FPV camera) for device {labeled_device.emission_id} ({labeled_device.label})"
		)
	...
	if not sensor_imu.connected:
		raise RuntimeError(
			f"Could not connect to direct IMU sensor for device {labeled_device.emission_id} ({labeled_device.label})"
		)
